...
public class RecoverRotatedArray {
	...
	static int findRotationCount(ArrayList<Integer> nums) {
		...
		int end = n - 1;
		/*
		 * //Already sorted if (nums.get(start) <= nums.get(end)) { return 0; }
		 */
		while (start + 1 < end) {
			int mid = start + (end - start) / 2;
			int midElement = nums.get(mid);
			/*
			 * if (midElement <= nums.get(mid - 1) && midElement <= nums.get(mid
			 * + 1)) { return mid; }
			 */
	...
	static int findRotationCount(int[] a) {
		...
		int mid = 0;
		/*
		 * if (a[low] <= a[high]) { return 0; //it is already sorted }
		 */
		while (low + 1 < high) {
			mid = low + (high - low) / 2;
			/*
